function [ts, criticalPoints, interpVec] = corallinagemodel( inputData, varargin )
...
[ts, criticalPoints, interpVec] = linearAgeModel(inputData, ppy, p.Results.sensitivity*2.7, p.Results.endperiodicity);
...
if p.Results.numyears > 0
    ...
    while (i < maxIterations) && (abs(proximity) > tolerance)
        [ts, criticalPoints, interpVec] = linearAgeModel(inputData, ppy, sensitivity*2.7, p.Results.endperiodicity);
        proximity = p.Results.numyears - ts(end,1);
...
function [ts, criticalPoints, intermediateX] = linearAgeModel(data, pointsPerYear, splineSensitivity, requiredPointsPerYear)
